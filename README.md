
Hi there, I’m Iman 👋

@imanagic • AI & Computer Science student • Software & ML Systems

👨‍💻 About Me

🎓 Studying Computer Science at Lewis & Clark College, with a concentration in Theatre Dance  
💼 Building software, data, and machine learning systems through internships and applied projects  
🌱 Interested in AI agents, cybersecurity, and real-world system design  

🧰 Tools & Technologies

**Languages:**  
Python, JavaScript, Java, C, SQL

**AI & ML:**  
PyTorch, HuggingFace, LangChain, LangGraph

**Backend & Data:**  
FastAPI, Flask, REST APIs, Node.js, Express  
PostgreSQL, SQLite, Pandas

**Dev & Systems:**  
Git, Linux, Docker (dev), pytest, AWS SDK (boto3)

📚 Experience & Projects

- Built REST APIs and data pipelines in Python and SQL for production systems  
- Automated analytics and reporting workflows handling 1,000+ records  
- Developed ML pipelines using Hugging Face datasets and cloud storage  
- Created accessibility-focused prototypes using Google Maps APIs  
- Competed in National Cyber League challenges (RSA, SQLi, PCAP, enumeration)

📫 Let’s Connect

📧 iman@e-comm.ba  
🔗 [LinkedIn](https://www.linkedin.com/in/iman-agic-117326309/)

<!--
**imanagic/imanagic** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
